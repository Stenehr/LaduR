...
const DataTable: React.FC<IProps<any>> = ({ color, rowContent, source, header }) => {
    ...
    const renderTd = (item: any, headerItem: IDataTableHeaderItem<any>) => {
        ...
        if (headerItem.format) fieldValue = headerItem.format(item);
        ...
        } else fieldValue = item[headerItem.field];
                ...
                <Table.Header>
                    <Table.Row>
                        ...
                        {header.map((headerItem, index) => (
                            <Table.HeaderCell key={index} style={headerItem.width ? { width: headerItem.width } : {}}>
                                {headerItem.header}
                            </Table.HeaderCell>
                        ))}
                    </Table.Row>
                        ...
                        <Fragment key={index}>
                            <Table.Row positive={isOpened(item)}>
                                {rowContent && (
                                    <Table.Cell>
                                        <Icon
                                            onClick={() => openCloseDetails(item)}
                                            link
                                            name={isOpened(item) ? "minus square" : "plus square"}
                                        />
                                    </Table.Cell>
                                )}
                                {header.map((headerItem, index) => renderTd(item, headerItem))}
                            </Table.Row>
                            {isOpened(item) && (
                                <Table.Row>
